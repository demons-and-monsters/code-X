...
void ForceSearchSolvor::solve(char *topo[5000], int edge_num, char *demand)
{   
    ...
    force_search(node_set[S], 0, path);
...
void ForceSearchSolvor::force_search(Node* node, int distance, std::vector<Edge*>& path) {
    ...
    for (int i = 0; i < node->outters.size(); i++) {
        ...
        force_search(to, distance + node->outters[i]->weight, path);
        to->visited = false;
        path.pop_back();
        new_line_flag = true;
    }
...
bool ForceSearchSolvor::read_topo(char* topo[5000], 
        int edge_num,
...
bool ForceSearchSolvor::read_demand(char* demand, 
